...
if __name__ == '__main__':
    ...
    total_params = sum(param.numel() for param in dinov2_vits14.parameters())
    ...
    for filename in tqdm(filenames):
        ...
        feature_data_path = str(Path(args.outdir).resolve().joinpath(image_fn)) + "-dinov2.npy"
        ...
        with torch.no_grad():
            # dinov2 vits14
            dinov2_features = dinov2_vits14.get_intermediate_layers(image, n=1)[0]
            dinov2_features = torch.mean(dinov2_features, dim=2).squeeze(0)
            seq_h = int(image.shape[2] / 14)
            seq_w = int(image.shape[3] / 14)
            dinov2_features = dinov2_features.reshape(seq_h,seq_w).cpu().numpy()

            np.save(feature_data_path, dinov2_features)
